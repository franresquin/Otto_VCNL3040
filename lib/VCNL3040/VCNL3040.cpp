/*
  This library has been adapted from the SPARKFUN library for the 
  VCNL4040 distance sensor.

  Author: Francisco Resquin @ Ottobock

  ---- VCNL4040 information ----
...
const uint8_t VCNL3040_ADDR = 0x60; //7-bit unshifted I2C address of VCNL4040
...
//Class constructor
VCNL3040::VCNL3040(void) {}

//Check comm with sensor and set it to default init settings
boolean VCNL3040::begin(TwoWire &wirePort) {
...
boolean VCNL3040::isConnected(void) {
  _i2cPort->beginTransmission((uint8_t)VCNL3040_ADDR);
...
void VCNL3040::setIRDutyCycle(uint16_t dutyValue)
{
  ...
  bitMask(VCNL3040_PS_CONF1, LOWER, VCNL3040_PS_DUTY_MASK, dutyValue);
}
...
void VCNL3040::setProxInterruptPersistance(uint8_t persValue)
{
  bitMask(VCNL3040_PS_CONF1, LOWER, VCNL3040_PS_PERS_MASK, persValue);
}
...
/*
void VCNL3040::setAmbientInterruptPersistance(uint8_t persValue)
{
  bitMask(VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_PERS_MASK, persValue);
}

void VCNL3040::enableAmbientInterrupts(void)
{
  bitMask(VCNL4040_ALS_CONF, LOWER, VCNL4040_ALS_INT_EN_MASK, VCNL4040_ALS_INT_ENABLE);
}
void VCNL3040::disableAmbientInterrupts(void)
{
...
void VCNL3040::setAmbientIntegrationTime(uint16_t timeValue)
{
...
void VCNL3040::setProxIntegrationTime(uint8_t timeValue)
{
  ...
  bitMask(VCNL3040_PS_CONF1, LOWER, VCNL3040_PS_IT_MASK, timeValue);
}

// Power on the prox sensing portion of the device //
void VCNL3040::powerOnProximity(void)
{
  bitMask(VCNL3040_PS_CONF1, LOWER, VCNL3040_PS_SD_MASK, VCNL3040_PS_SD_POWER_ON);
}

// Power off the prox sensing portion of the device //
void VCNL3040::powerOffProximity(void)
{
  bitMask(VCNL3040_PS_CONF1, LOWER, VCNL3040_PS_SD_MASK, VCNL3040_PS_SD_POWER_OFF);
}

// Sets the proximity resolution //
void VCNL3040::setProxResolution(uint8_t resolutionValue)
{
	if(resolutionValue > 16 - 1) resolutionValue = VCNL3040_PS_HD_16_BIT;
	else resolutionValue = VCNL3040_PS_HD_12_BIT;
	
  bitMask(VCNL3040_PS_CONF2, UPPER, VCNL3040_PS_HD_MASK, resolutionValue);
}

//Sets the proximity interrupt type
void VCNL3040::setProxInterruptType(uint8_t interruptValue)
{
  bitMask(VCNL3040_PS_CONF2, UPPER, VCNL3040_PS_INT_MASK, interruptValue);
}
...
void VCNL3040::enableSmartPersistance(void)
{
  bitMask(VCNL3040_PS_CONF3, LOWER, VCNL3040_PS_SMART_PERS_MASK, VCNL3040_PS_SMART_PERS_ENABLE);
}
void VCNL3040::disableSmartPersistance(void)
{
  bitMask(VCNL3040_PS_CONF3, LOWER, VCNL3040_PS_SMART_PERS_MASK, VCNL3040_PS_SMART_PERS_DISABLE);
}
...
void VCNL3040::enableActiveForceMode(void)
{
  bitMask(VCNL3040_PS_CONF3, LOWER, VCNL3040_PS_FOR_MASK, VCNL3040_PS_FOR_ENABLE);
}
void VCNL3040::disableActiveForceMode(void)
{
  bitMask(VCNL3040_PS_CONF3, LOWER, VCNL3040_PS_FOR_MASK, VCNL3040_PS_FOR_DISABLE);
}

//Set trigger bit so sensor takes a force mode measurement and returns to standby
void VCNL3040::takeSingleProxMeasurement(void)
{
  bitMask(VCNL3040_PS_CONF3, LOWER, VCNL3040_PS_FOR_TRIG_MASK, VCNL3040_PS_FOR_TRIG_ONETIME);
}

/*
//Enable the white measurement channel
void VCNL3040::enableWhiteChannel(void)
{
  bitMask(VCNL3040_PS_MS, UPPER, VCNL4040_WHITE_EN_MASK, VCNL4040_WHITE_ENABLE);
}
void VCNL3040::disableWhiteChannel(void)
{
...
void VCNL3040::enableProxLogicMode(void)
{
  bitMask(VCNL3040_PS_MS, UPPER, VCNL3040_PS_MS_MASK, VCNL3040_PS_MS_ENABLE);
}
void VCNL3040::disableProxLogicMode(void)
{
  bitMask(VCNL3040_PS_MS, UPPER, VCNL3040_PS_MS_MASK, VCNL3040_PS_MS_DISABLE);
}

//Set the IR LED sink current to one of 8 settings
void VCNL3040::setLEDCurrent(uint8_t currentValue)
{
	...
	bitMask(VCNL3040_PS_MS, UPPER, VCNL3040_LED_I_MASK, currentValue);
}

//Set the proximity sensing cancelation value - helps reduce cross talk
//with ambient light
void VCNL3040::setProxCancellation(uint16_t cancelValue)
{
  writeCommand(VCNL3040_PS_CANC, cancelValue);
}

/*
//Value that ALS must go above to trigger an interrupt
void VCNL3040::setALSHighThreshold(uint16_t threshold)
{
  writeCommand(VCNL4040_ALS_THDH, threshold);
}

//Value that ALS must go below to trigger an interrupt
void VCNL3040::setALSLowThreshold(uint16_t threshold)
{
...
void VCNL3040::setProxHighThreshold(uint16_t threshold)
{
  writeCommand(VCNL3040_PS_THDH, threshold);
}

//Value that Proximity Sensing must go below to trigger an interrupt
void VCNL3040::setProxLowThreshold(uint16_t threshold)
{
  writeCommand(VCNL3040_PS_THDL, threshold);
}

//Read the Proximity value
uint16_t VCNL3040::getProximity()
{
  return (readCommand(VCNL3040_PS_DATA));
}

/*
//Read the Ambient light value
uint16_t VCNL3040::getAmbient()
{
  return (readCommand(VCNL4040_ALS_DATA));
}

//Read the White light value
uint16_t VCNL3040::getWhite()
{
...
uint16_t VCNL3040::readCommand(uint8_t commandCode)
{
  _i2cPort->beginTransmission(VCNL3040_ADDR);
  ...
  _i2cPort->requestFrom((uint8_t)VCNL3040_ADDR, (uint8_t)2);
...
boolean VCNL3040::writeCommand(uint8_t commandCode, uint16_t value)
{
  _i2cPort->beginTransmission(VCNL3040_ADDR);
  _i2cPort->write(commandCode);
...
boolean VCNL3040::writeCommandLower(uint8_t commandCode, uint8_t newValue)
{
...
boolean VCNL3040::writeCommandUpper(uint8_t commandCode, uint8_t newValue)
{
  uint16_t commandValue = readCommand(commandCode);
...
uint8_t VCNL3040::readCommandLower(uint8_t commandCode)
{
  uint16_t commandValue = readCommand(commandCode);
...
uint8_t VCNL3040::readCommandUpper(uint8_t commandCode)
{
  uint16_t commandValue = readCommand(commandCode);
...
void VCNL3040::bitMask(uint8_t commandAddress, boolean commandHeight, uint8_t mask, uint8_t thing)
{
